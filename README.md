...
啟動 UI/UX 視覺設計懶人包 SOP (整合 a-plan 雙煞車、倒轉時光機、奧客測試與防當機黑名單)。
使用者輸入 `/ui-ux-plan` 時觸發，指引視覺風格匹配、配色方案、Google Fonts 選擇、Artifacts 假畫面試玩、極限破壞測試與設計系統持久化。

## ✨ 核心特色
1. 🗣️ **全白話溝通（麻瓜說書人模式）**：不講生硬名詞，用生活化比喻解說設計觀念。
2. 🎨 **UI UX Pro Max 數據庫支援**：自動配對 79 種 UI 風格、192 套配色與 74 組 Google Fonts。
3. 🕹️ **雙預覽試玩機制 (Artifacts)**：先在聊天室產出可互動 HTML 試玩，確認過關後才寫入專案。
4. 🛡️ **鋼鐵安全機制**：
   - 🆘 `/wait-what` 求救煞車鍵
